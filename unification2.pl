...
%
% untrace_unif unifie le programme en affichant aucune trace d execution
...
unifie(P) :-
    ...
    echo('(a) choix_premier'), nl,
    echo('(b) choix_pondéré'), nl,
    read(InputStrategie),
    (InputStrategie == a -> S = premier;
    InputStrategie == b -> S = pondere;
    S = premier),
    echo('Choix de la stratégie : '),
    echo(S), nl,
    echo('Afficher la trace d\'execution ? (y/n)'), nl,
